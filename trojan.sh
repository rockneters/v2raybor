...
#definition操作变量, 0 for 否, 1 for Yes
HELP=0
...
installTrojan(){
    ...
    echo "Orthodox 下载manage program`colorEcho $BLUE $LASTEST_VERSION`版本..."
	...
	source ~/.${SHELL_WAY}rc
    if [[ $UPDATE == 0 ]];then
        colorEcho $GREEN "Installtrojanmanage Program succeeded!\n"
        echo -e "运行命令`colorEcho $BLUE trojan`可进行trojanmanage \n"
	...
	expect {
		"Certificate method" { send "1"; exp_continue }
		"Book domain" { send "$domain\n"; exp_continue }
		"ysql connection" { sleep 1; send "1"; exp_continue }
		"Username" { send "root\n"; exp_continue }
		"definition password " { send "$uuid\n"; exp_continue }
		"generate json" { sleep 1; send "\n" }
	}
	expect eof
EOF
    else
        ...
        colorEcho $GREEN "renew trojanmanage Program succeeded!\n"
    fi
    setupCron
    [[ $SHOW_TIP == 1 ]] && echo "Browser access'`colorEcho $BLUE https://Domain name can be online trojan and more users Management"
}

main(){
    [[ ${HELP} == 1 ]] && help && return
    [[ ${REMOVE} == 1 ]] && removeTrojan && return
    [[ $UPDATE == 0 ]] && echo "Orthodox Install trojan manage program.." || echo "Updating trojanmanage program.."
    checkSys
